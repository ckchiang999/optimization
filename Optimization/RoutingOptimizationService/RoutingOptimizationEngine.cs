...
namespace RoutingOptimizationService
{
    public class RoutingOptimizationEngine : IRoutingOptimization
    {
        ...
        /// A <see cref="RoutingResponseDto"/>
        /// </returns>
        public RoutingResponseDto SolveResourceConstrainedTimeWindowVehicleRoutingProblem()
        {
            RoutingResponseDto response = new();
            ...
            // Amount of time to load vehicle
            const int vehicleLoadTime = 5;

            // Amount of time to unload vehicle
            const int vehicleUnloadTime = 5;

            // Maximum number of vehicles that can load and unload at a location
            const int depotCapacity = 2;
            ...
            var intervals = new IntervalVar[vehicleNumber * 2];
            for (int i = 0; i < vehicleNumber; i++)
            {
            ...
            // Vector containing the relative amounts of space required by each vehicle
            // during loading / unloading.  We assume all vehicles require the same amount of space
            // so it contains all ones.
            long[] depotUsage = Enumerable.Repeat<long>(1, intervals.Length).ToArray();

            // Create constraint that at most two vehicles can loaded or unloaded at the same time.
            solver.Add(solver.MakeCumulative(intervals, depotUsage, depotCapacity, "depot"));
        ...
        /// A <see cref="RoutingResponseDto"/>
        /// </returns>
        public RoutingResponseDto SolvePenaltyDefinedVehicleRoutingProblem()
        {
            RoutingResponseDto response = new();
            ...
            long[] demands = { 0, 1, 1, 3, 6, 3, 6, 8, 8, 1, 2, 1, 2, 6, 6, 8, 8 };
            long[] vehicleCapacities = { 15, 15, 15, 15 };
            int vehicleNumber = 4;
            int depot = 0;
            ...
            // Create the routing index manager
            // The manager does conversions of the solver's internal indices to the numbers for locations.
            var manager = new RoutingIndexManager(distanceMatrix.GetLength(0), vehicleNumber, depot);
            ...
            // Allow to drop nodes
            long penalty = 1000; // set penalty to be higher than the sum of all distances between locations
            ...
            for (int i = 0; i < vehicleNumber; i++)
            {
                ...
                {
                    var startLocationNode = manager.IndexToNode((int)index);
                    var load = demands[startLocationNode];
                    routeLoad += load;
                    Debug.Write($"{startLocationNode} Load({routeLoad} -> ");
                    var previousIndex = index;
                    index = solution.Value(routing.NextVar(index));
                ...
                Debug.WriteLine(manager.IndexToNode((int)index));
                Debug.WriteLine($"Distance of the route: {routeDistance}m");
                route.TotalDistance = routeDistance;
                route.TotalLoad = routeLoad;
